# Production Build and Package Script - Docker Version
# Purpose: Build backend JAR and frontend static files, package to deploy directory with Docker support
...
$backupsDir = "$deployDir/backups"
New-Item -ItemType Directory -Path $backupsDir -Force | Out-Null

# Copy Docker files
Write-Host "Copying Docker configuration files..." -ForegroundColor Yellow
Copy-Item -Path "Dockerfile" -Destination "$backendDir/Dockerfile" -ErrorAction SilentlyContinue
Copy-Item -Path "docker-compose.yml" -Destination "$deployDir/docker-compose.yml" -ErrorAction SilentlyContinue
Copy-Item -Path "docker-compose-server.yml" -Destination "$deployDir/docker-compose-server.yml" -ErrorAction SilentlyContinue
Copy-Item -Path "nginx-docker.conf" -Destination "$deployDir/nginx.conf" -ErrorAction SilentlyContinue
Copy-Item -Path "deploy-docker-server.sh" -Destination "$scriptsDir/deploy.sh" -ErrorAction SilentlyContinue
Copy-Item -Path "Docker部署说明.md" -Destination "$deployDir/README-Docker.md" -ErrorAction SilentlyContinue

# Create server management scripts
Write-Host "Creating server management scripts..." -ForegroundColor Yellow

$utf8NoBom = New-Object System.Text.UTF8Encoding $false

$startScript = "#!/bin/bash`ncd `"`$(dirname `"`$(dirname `"`${BASH_SOURCE[0]}`")`")`"`nif [ -f `"docker-compose-server.yml`" ]; then COMPOSE_FILE=`"docker-compose-server.yml`"; else COMPOSE_FILE=`"docker-compose.yml`"; fi`necho `"Starting Docker containers...`"`ndocker-compose -f `$COMPOSE_FILE up -d`necho `"Services started!`"`ndocker-compose -f `$COMPOSE_FILE ps"
[System.IO.File]::WriteAllText("$scriptsDir/start.sh", $startScript, $utf8NoBom)

$stopScript = "#!/bin/bash`ncd `"`$(dirname `"`$(dirname `"`${BASH_SOURCE[0]}`")`")`"`nif [ -f `"docker-compose-server.yml`" ]; then COMPOSE_FILE=`"docker-compose-server.yml`"; else COMPOSE_FILE=`"docker-compose.yml`"; fi`necho `"Stopping Docker containers...`"`ndocker-compose -f `$COMPOSE_FILE down`necho `"Services stopped!`""
[System.IO.File]::WriteAllText("$scriptsDir/stop.sh", $stopScript, $utf8NoBom)

$restartScript = "#!/bin/bash`ncd `"`$(dirname `"`$(dirname `"`${BASH_SOURCE[0]}`")`")`"`nif [ -f `"docker-compose-server.yml`" ]; then COMPOSE_FILE=`"docker-compose-server.yml`"; else COMPOSE_FILE=`"docker-compose.yml`"; fi`necho `"Restarting Docker containers...`"`ndocker-compose -f `$COMPOSE_FILE restart`necho `"Services restarted!`"`ndocker-compose -f `$COMPOSE_FILE ps"
[System.IO.File]::WriteAllText("$scriptsDir/restart.sh", $restartScript, $utf8NoBom)

$logsScript = "#!/bin/bash`ncd `"`$(dirname `"`$(dirname `"`${BASH_SOURCE[0]}`")`")`"`nif [ -f `"docker-compose-server.yml`" ]; then COMPOSE_FILE=`"docker-compose-server.yml`"; else COMPOSE_FILE=`"docker-compose.yml`"; fi`nif [ -z `"`$1`" ]; then`n    docker-compose -f `$COMPOSE_FILE logs -f`nelse`n    docker-compose -f `$COMPOSE_FILE logs -f `$1`nfi"
[System.IO.File]::WriteAllText("$scriptsDir/logs.sh", $logsScript, $utf8NoBom)

$statusScript = "#!/bin/bash`ncd `"`$(dirname `"`$(dirname `"`${BASH_SOURCE[0]}`")`")`"`nif [ -f `"docker-compose-server.yml`" ]; then COMPOSE_FILE=`"docker-compose-server.yml`"; else COMPOSE_FILE=`"docker-compose.yml`"; fi`necho `"Service status:`"`ndocker-compose -f `$COMPOSE_FILE ps"
[System.IO.File]::WriteAllText("$scriptsDir/status.sh", $statusScript, $utf8NoBom)

Write-Host "Docker configuration and scripts created!" -ForegroundColor Green
